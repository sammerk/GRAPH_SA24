...
mod3 <- brm(Liegestutzen ~ Geschlecht + Alter_n, 
            data = data_lieg,
            family = poisson())
...
mod4 <- brm(Liegestutzen ~ Geschlecht + Alter_n, 
            data = data_lieg)
...
# Ideen:
# 1. Vergleich der Poisson-Verteilung mit der Normalverteilung
# 2. Vergleich der Poisson-Verteilung mit verschiedenen Linkfunktionen
```
...
$$
\begin{aligned}
\text{Modell für weibliche Kinder: } \lambda_i &= e^{b_0} \\
\text{Modell für männliche Kinder: } \lambda_i &= e^{b_0 + b_1} = e^{b_0} \cdot e^{b_1}
\end{aligned}
$$
